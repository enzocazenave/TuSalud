...
export default function MedicalRecordView() {
  ...
  const renderMedicalRecord = ({item}: {item: any}) => {
    ...
    return (
      <View className="bg-secondary dark:bg-gray-800 rounded-[20px] p-5 gap-4 relative">
        <TouchableOpacity
          onPress={openUrl}
          className="absolute bottom-2 right-2 p-3 rounded-2xl bg-primary/20">
          <FileText
            size={20}
        ...
        <View className="flex-row gap-4 items-start">
          ...
          <View className="gap-8 justify-between flex-1">
            <View>
              ...
              <Text className="text-primary dark:text-secondary text-[15px]">
                {item?.record}
              </Text>
            </View>
            ...
            <View className="flex-row items-center gap-2">
              <Calendar
                ...
                color={theme === 'dark' ? '#9ACBD0' : '#006A71'}
              />
              <Text className="text-primary dark:text-secondary font-semibold text-md">
                {formattedDate}
              </Text>
            </View>
          </View>
