  ...
  const handleSaveChanges = async () => {
    ...
    if (hasPrepaidDataChanges()) {
      await updateUserPrepaid(
        {
          prepaidId: initialData.prepaidId,
          plan: initialData.plan,
          number: initialData.number,
        },
        {
          prepaidId: personalData.prepaidId,
          plan: personalData.plan,
    ...
    <ScrollView
      ...
      <Dropdown label="Obra Social">
        <View className="px-5 py-4 bg-quaternary gap-3">
          <SelectDropdown
            data={prepaids}
            defaultValue={prepaids.find(
              prepaid => prepaid.id === personalData.prepaidId,
            )}
            ...
            renderItem={(selectedItem, _, isSelected) => {
              return (
                <View
                  className={`px-4 py-3 border-b border-primary dark:border-gray-600 flex-row justify-between items-center gap-4 ${isSelected ? 'bg-tertiary' : ''}`}>
                  <Text
                    className={`text-primary dark:text-secondary font-semibold ${isSelected ? 'text-white' : ''}`}>
                    {selectedItem.name}
                  </Text>
                  {isSelected && (
            ...
            dropdownStyle={{
              borderRadius: 10,
              backgroundColor: theme === 'dark' ? '#1B1B1B' : '#9ACBD0',
            }}
          />
          <Input
            label="Plan Médico"
